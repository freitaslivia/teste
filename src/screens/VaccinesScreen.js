    ...
    const fetchVacinas = async () => {
      try {
        ...
        const response = await fetch(
          `https://yaso.azurewebsites.net/vacinas/${userId}`
        );
  ...
  const toggleItem = async (itemId) => {
    if (expandedItem === itemId) {
      ...
      if (!comprovantes[itemId]) {
        try {
          const response = await fetch(
            `https://yaso.azurewebsites.net/vacinas/comprovante/${itemId}`
          );
          const comprovanteUri = await response.json();
          setComprovantes((prev) => ({ ...prev, [itemId]: comprovanteUri }));
        } catch (error) {
          console.error(
            `Erro ao buscar comprovante para vacina ${itemId}:`,
            error
          );
        }
      }
            ...
            {expandedItem === item.id && (
              <View style={styles.itemContentContainer}>
                <Text style={styles.itemContent}>
                  Data de Aplicação: {item.dataAplicacao}
                </Text>
                <Text style={styles.itemContent}>
